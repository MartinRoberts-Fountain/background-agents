## You are executing the autonomous multi-agent workflow. Run all steps without waiting for user input. The user has walked away.
...
5. The tests will be failing at this point. The @apply agent is going to implement the
   implementation plan and needs information from the test agent on getting the tests to pass while
   addressing all the issue requirements and acceptance criteria. Tasks that can be done
