...
reason (RED), then hand off to `@make` for implementation (GREEN).
...
If you believe source code needs changes to be testable, report this to the caller — do not edit it
yourself.
...
Only `MISSING_BEHAVIOR` and `ASSERTION_MISMATCH` qualify as valid RED. Fix `TEST_BROKEN` before
reporting. Report `ENV_BROKEN` as BLOCKED.
